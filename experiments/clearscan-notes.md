...
```bash
uv run clearscan/tools/make_fixture.py clearscan/fixtures/control
```

Probe a PDF:

```bash
uv run clearscan/tools/verify_pdf.py probe clearscan/fixtures/control/control_scan.pdf
```
...
uv run clearscan/tools/verify_pdf.py compare \
  clearscan/fixtures/control/control_scan.pdf \
  clearscan/artifacts/some-output.pdf \
  --out-dir clearscan/reports/some-output
```
